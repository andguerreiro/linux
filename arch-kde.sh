...
# Combine removal and installation to reduce sudo calls
...
sudo ufw allow 631/{tcp,udp} >/dev/null   # Use brace expansion for ports
sudo ufw --force enable >/dev/null        # Use --force instead of piping "y"
...
# --- 4. System Tweaks ---
echo "Applying Systemd-boot and UDEV rules..."
# Simplified bootloader check
[ -f /boot/loader/loader.conf ] && sudo sed -i 's/^timeout.*/timeout 0/' /boot/loader/loader.conf

# UDEV rules for Razer
