...
# =============================================================================
# DEBIAN REPOSITORIES (required for NVIDIA)
# =============================================================================
...
[wireless]
label=NET:
command=sh -c 'if [ "$BLOCK_BUTTON" = 1 ]; then setsid kitty -e wavemon >/dev/null 2>&1 & fi; iface=$(iw dev 2>/dev/null | awk "$1==\"Interface\"{print \$2; exit}"); [ -z "$iface" ] && { echo OFFLINE; exit; }; ssid=$(iw dev "$iface" link 2>/dev/null | awk "/SSID/ {print \$2}"); [ -z "$ssid" ] && echo OFFLINE || echo "$ssid"'
interval=5
