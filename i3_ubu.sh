...
echo "=== Ubuntu Server 24.04 — i3 Workflow (Volume Fix) ==="
...
# --------------------------------------------------
# 5. Volume Helper Script (The Fix)
# --------------------------------------------------
mkdir -p ~/.config/i3blocks
cat <<'EOF' > ~/.config/i3blocks/volume.sh
#!/bin/sh
# Use wpctl (native PipeWire) if available, otherwise pactl
if command -v wpctl >/dev/null; then
    ...
    MUTE=$(pactl get-sink-mute @DEFAULT_SINK@ | grep -q "yes" && echo "yes")
fi
...
# Give the system time to load audio drivers before first update
...
echo "=== SETUP COMPLETE. REBOOT AND TRY YOUR VOLUME KEYS ==="
